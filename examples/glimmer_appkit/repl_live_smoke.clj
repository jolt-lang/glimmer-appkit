(ns glimmer-appkit.repl-live-smoke
  ...
  re-render must hop onto the AppKit main loop via the CFRunLoopSource scheduler
  (gui-loop-running? marshalling in glimmer.core), NOT reconcile inline on the
  worker thread — on macOS that off-main-thread AppKit mutation aborts.
...
(defn -main [& _]
  (let [result (promise)]
    ...
    ;; thread. Give the app time to mount + enter the AppKit run first.
    (future
      (Thread/sleep 600)
